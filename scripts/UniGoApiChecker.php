...
namespace UniGoScripts;

require UniGoApiChecker::getComposerRootPath().'/vendor/autoload.php';

use Composer\IO\IOInterface;
use Composer\Script\Event;
use Unisender\UniGoClient;

class UniGoApiChecker
{
    /**
    ...
    {
        $this->client = new UniGoClient($args[1], $args[0]);
        $this->config = $config;
        $this->args = $args;
    }

        /**
         * Check Unisender Go API methods.
         *
        ...
        public static function testApi(Event $event)
        {
            ...
            if (\count($args) < 2) {
                $io->write('Please enter HOSTNAME and APIKEY parameters. Example: composer test UNIGO-HOSTNAME UNIGO-API-KEY', true);
                exit(1);
            }
            if (empty($config)) {
            ...
            $api_checker = new UniGoApiChecker($args, $config);
      ...
      /**
       * Test Unisender Go API methods, that are not implemented in SDK yet.
       *
       * @return void
       */
